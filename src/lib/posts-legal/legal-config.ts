...
const postsDirectory = path.join(process.cwd(), "content/posts-legal");

type PostAttributes = z.infer<typeof LegalSchema>;
...
export const getPosts = async (tags?: string[]) => {
  ...
  for await (const fileName of fileNames) {
    ...
    const result = LegalSchema.safeParse(matter.attributes);
